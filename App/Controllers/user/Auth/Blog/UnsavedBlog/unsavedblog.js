...
async function AlreadySavedBlog(req, res, next) {
  try {
    ...
    const sql = `
       SELECT id
       FROM saved_blog
       WHERE blog_id = ? AND student_id = ? AND saved_status='0'
    `;
    ...
    connection.query(sql, values, (error, results) => {
      if (error) {
        ...
        if (results.length > 0) {
          return returnServerRes(res, 409, false, "You Already Unsaved this blog");
        } else {
          return next();
        }
      }
        ...
        if (results.length > 0) {
          ...
          if (likeStatus === '1') {
            ...
            connection.query(updateSql, updateValues, (error, updateResults) => {
              if (error) {
                ...
                const successMsg = "Again Unsaved the blog";
          ...
          connection.query(insertSql, insertValues, (error, insertResults) => {
            if (error) {
              ...
              const successMsg = "saved the blog";
